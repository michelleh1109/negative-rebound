...
per2 <- per2 %>% filter(GT1JBLWK %in% "02") %>% filter(WKFTPT %in% "01")

# both ppl do not attend school
per2 <- per2 %>% filter(SCHTYP %in% c("-1", "03"))

# both ppl. can drive
per2 <- per2 %>% filter(DRIVER %in% "01")
...
```
Above is a boxplot of the windsorized "VARMILES." As expected, most of the data is right skewed, meaning most people do not drive a lot of miles for recreational purposes. It is suprising however that some outliers indivudals drive over 100,000 miles.
...
colnames(KS_matrix) <- c("one_Car", "two_Car", "three_Car", "four_Car")
rownames(KS_matrix) <- c("one_Car", "two_Car", "three_Car", "four_Car")
```
```{r}
library(reshape2)

ksdf = as.data.frame(KS_matrix)
colnames(ksdf) <- c("one_car", "two_car", "three_car", "four_car")
rownames(ksdf) <- c(1,2,3,4)
meltksdf <- melt(ksdf)

ggplot(ksdf, aes(x = variable, y = value, fill = value)) +
  geom_tile() +
...
```
Comparing the types of vehicles by most to least driven. 
01=Automobile/Car/StationWagon 02=Van (Mini/Cargo/Passenger) 03=SUV (Santa Fe, Tahoe, Jeep, etc.) 04=Pickup Truck 05=Other Truck 06=RV (Recreational Vehicle)

In 4 car households, a larger proportion of their last car is a recreational or multi-purpose larger vehicle including van or truck that is least drive. Compared to 2 person households, virtually no vans or trucks are in the vehicle mix. 
...
# only households in Lifecycle w/ 2 working adults and no kids
hh_2per <- hh_2per %>% filter(LIF_CYC %in% "02")
hh_2per <- hh_2per %>% filter(WRKCOUNT %in% "02")
